...
function ImageCarousel({ images, onZoom }: { images: string[], onZoom: (src: string) => void }) {
  ...
  return (
    <div className="carousel-container">
      <div className="carousel-zoom-hint">双击预览图片</div>
      {images.length > 1 && (
        <>
          <button 
...
export function WorkExperience() {
  ...
  const items = RESUME_DATA.workExperience || [];
      ...
      {items.map((item, idx) => (
        <div key={idx} className="glass-panel" style={{ padding: '40px', borderRadius: '2px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-main)', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', width: '100%', flexWrap: 'wrap' }}>
              <span style={{ flex: '1', textAlign: 'left' }}>{item.company}</span>
              <span style={{ flex: '1', textAlign: 'center', color: 'var(--accent)' }}>{item.role}</span>
              <span style={{ flex: '1', textAlign: 'right', fontWeight: 'normal', fontSize: '1.1rem', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>{item.period}</span>
            </h3>
          </div>
